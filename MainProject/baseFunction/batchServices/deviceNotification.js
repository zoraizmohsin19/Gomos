...
function translateBulkMethod(ConvertedSensors, message, DeviceName, payloadId, subCustCd, custCd, date, db, res, mac, assetId, token,remark) {
  try {
    var dataToStore = [];
   for(let i =0 ; i < message.length ; i++){
    let finalSensors = {};
    finalSensors["payloadId"] = payloadId;
    let msgClientKeys = Object.keys(message[i]);
    let sensorsNotFound = '';

    for (let k = 0; k < msgClientKeys.length; k++) {
      let configNMValues = {};
      // Key comparisons and value replacements are done here
      if (ConvertedSensors[msgClientKeys[k]]) {
        let configNm = ConvertedSensors[msgClientKeys[k]];
        let businessValue = message[i][msgClientKeys[k]];
    ...
    if (sensorsNotFound.length == 0) {
      ...
      let nowDateTime = new Date(new Date().toISOString())
   dataToStore.push({
  ...
  gomos.gomosLog( logger,gConsole,TRACE_PROD, "Entry  This is Message Ready to Insert", dataToStore);

     // successUpdate(DeviceName, payloadId, subCustCd, custCd, Date, message, finalSensors, res, db, "success", mac, assetId,remark)
    
    db.collection("Alerts")
    .insertMany(dataToStore, function (err, result) {
      if (err) {
...
exports.endPointMiddelayerFnForBulkWithRemark = function (urlConn, dbName, res, custCd, subCustCd, DeviceName, payloadId, Date, message, token, remark) {
  urlConn = urlConn;
  dbName = dbName;
   gomos.gomosLog( logger,gConsole,TRACE_DEBUG, "Post method hit by endpoint thier value" + ":" + DeviceName + ":" + payloadId + ":" + subCustCd + ":" + payloadId + ":" + DeviceName + ":" + custCd, message);
   gomos.gomosLog( logger,gConsole,TRACE_DEBUG, "remarkObj", remark);
                            ...
                            if (result.length > 0) {
                              ...
                              if (sensorNms != undefined) {
                                ...
                                for (var k = 0; k < sensorKeys.length; k++) {
                                 ...
                                 gomos.gomosLog( logger,gConsole,TRACE_DEBUG, "ConvertedSensors  sensors key value", ConvertedSensors);
                                 translateBulkMethod(ConvertedSensors, message, DeviceName, payloadId, subCustCd, custCd, Date, db, res, mac, assetId, token,remark);
                              }
                            }
                            else {
                              errorUpdate(DeviceName, payloadId, subCustCd, custCd, Date, message, db, res, "Payload [" + payloadId + "] not Present",remark);
